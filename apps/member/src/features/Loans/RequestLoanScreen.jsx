...
export default function RequestLoanScreen({ user }) {
  ...
  const [activeLoans, setActiveLoans] = useState(null); // null = loading
  ...
  // Real-time active/pending loans subscription
  useEffect(() => {
    if (!user?.uid) return;
    const q = query(
      collection(db, "loans"),
  ...
  const amountExceedsLimit = Number.isFinite(parsedAmount) && parsedAmount > creditLimit && creditLimit > 0;
  ...
  async function handleSubmit(e) {
    ...
    if (!Number.isFinite(parsedAmount) || parsedAmount < 100) {
      setError("Minimum loan amount is 100 BIF.");
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await requestLoan({ amount: parsedAmount });
      setResult(res.data);
      setAmount("");
    } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-10">
      <div className="max-w-lg mx-auto px-4 pt-6">
        ...
        {/* Blocked: active or pending loan exists */}
        {hasActiveLoan ? (
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <button
              type="submit"
              disabled={submitting || !amount || amountExceedsLimit}
    ...
    <div className="rounded-xl bg-amber-50 border border-amber-200 px-5 py-5 space-y-3">
      ...
      {loan && (
        <div className="rounded-lg bg-white border border-amber-100 divide-y divide-amber-50 text-sm">
          <LoanDetailRow label="Amount" value={`${Number(loan.amount || 0).toLocaleString("en-US")} BIF`} />
          {loan.remainingDue != null && (
            <LoanDetailRow label="Remaining" value={`${Number(loan.remainingDue).toLocaleString("en-US")} BIF`} />
          )}
          {loan.dueDate && (
...
function LoanDetailRow({ label, value }) {
  return (
    <div className="px-4 py-2.5 flex justify-between items-center">
  ...
  return (
    <div className={`rounded-xl border px-5 py-6 space-y-4 ${
      ...
      <div className={`rounded-lg bg-white border divide-y text-sm ${
        ...
        <LoanDetailRow label="Status" value={result?.status || "—"} />
        {result?.loanId && <LoanDetailRow label="Reference" value={result.loanId.slice(0, 12) + "…"} />}
        {result?.rejectionReason && (
          <LoanDetailRow label="Reason" value={result.rejectionReason} />
        )}
      </div>
